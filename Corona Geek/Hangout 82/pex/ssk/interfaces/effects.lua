...
local function doShrink( obj, scale, time, myEasing )
	local scale = scale or 0.85
	local time = time or 50
	local myEasing = myEasing or easing.linear
	transition.to( obj, { xScale = scale, yScale = scale, time = 50 })
	if(obj.label) then
		transition.to( obj.label, { xScale = scale, yScale = scale, time = 50 })
	end
end

local function doGrow( obj, time, myEasing  )
	local time = time or 50
	local myEasing = myEasing or easing.linear
