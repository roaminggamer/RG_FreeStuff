...
--
local player = newRect( layers.content, centerX, centerY + 100, { size = 60, fill = _W_, moveLeft = 0, moveRight = 0 }, { isFixedRotation = true, bounce = 0.1 } )
--
...
local function createGameObject( x, y, objectType )
	...
	if( objectType == "platform" ) then
		obj = newRect( layers.background, x, y, 
			            { w = 200, h = 40, anchorY = 0, fill = _G_, isPlatform = true }, 
			            { bodyType = "static", bounce = 0 } )	
		lastX = x
		lastY = y

	elseif( objectType == "spring" ) then
		obj = newRect( layers.background, x + mRand(-40, 40), y, 
			            { w = 40, h = 40, anchorY = 1, yScale = 0.2, fill = _BRIGHTORANGE_, isSpring = true }, 
			            { bodyType = "static", bounce = 0, shape = {-20,10, 20, 10, 20, 20, -20, 20 } } )					
	
	elseif( objectType == "pickup" ) then
		obj = newCircle( layers.background, x + mRand(-40, 40), y, 
			            { radius = 20, h = 40, anchorY = 1, fill = _Y_, isPickup = true }, 
			            { bodyType = "static", radius = 20, isSensor = true } )	
	
	elseif( objectType == "danger" ) then
		obj = newRect( layers.background, x + mRand(-70, 70), y, 
			            { w = 40, h = 40, anchorY = 1, fill = _R_, isDanger = true }, 
			            { bodyType = "static", bounce = 0 } )
