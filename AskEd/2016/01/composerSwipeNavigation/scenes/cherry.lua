...
function scene:create( event )
	...
	local label = display.newText( sceneGroup, "C is for Cherry", centerX, centerY-100, native.systemFont, 30 )
	label:setFillColor( 1,0,0  )

	-- Picture of fruit
	local pic = display.newImageRect( sceneGroup, "images/cherry.png", 200, 200 )
	pic.x = centerX
	pic.y = centerY + 50
	...
	local function onAction( self, event )
		...
		pic.x = centerX - 25
		transition.to( pic, { x = centerX + 25, time = 1000 } )
		transition.to( pic, { delay = 1000, x = centerX, time = 3000, transition = easing.outElastic } )
	end
