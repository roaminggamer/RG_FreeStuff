...
function scene:create( event )
	...
	local back = display.newImageRect( sceneGroup, "images/back2.jpg", 320, 480 )
	back.x = centerX
	back.y = centerY
	if(isLandscape) then back.rotation = 90 end

	-- Create a label showing which scene this is
	local label = display.newText( sceneGroup, "B is for Banana", centerX, centerY-100, native.systemFont, 30 )
	label:setFillColor( 1,1,0 )

	-- Picture of fruit
	local pic = display.newImageRect( sceneGroup, "images/banana.png", 240, 240 )
	pic.x = centerX
	pic.y = centerY + 50
	...
	local function onAction( self, event )
		...
		pic.xScale = 0.75
		pic.yScale = 0.75
		transition.to( pic, { xScale = 1, yScale = 1, time = 3000, transition = easing.outElastic } )
	end
	PushButton( sceneGroup, centerX, centerY + fullh/2 - 30, "Action", onAction )

	-- Create a swiper on this page ONLY
	local swiper = composer.createSwipe( sceneGroup, {alwaysTo = "back"} )
end
