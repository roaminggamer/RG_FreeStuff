...
local tilesInfo 	= require "tiles"
local tilesSheet 	= graphics.newImageSheet("tiles.png", tilesInfo:getSheet() )
...
for i = 1, (2*display.actualContentWidth)/size do
	for j = 1, (2*display.actualContentHeight)/size do
		local tmp = display.newImageRect( group, tilesSheet, math.random(1,2), size, size )
