...
local function fixedCamera( player, layers, params )	
	...
	if(not lockX) then layers.lx = player.x end
	if(not lockY) then layers.ly = player.y end

	layers.world.enterFrame = function( event )
		if( not isDisplayObject( layers ) ) then return end
		if( not isDisplayObject( player ) ) then return end
		local dx = 0
		local dy = 0
		if(not lockX) then dx = player.x - layers.lx end		
		if(not lockY) then dy = player.y - layers.ly end
