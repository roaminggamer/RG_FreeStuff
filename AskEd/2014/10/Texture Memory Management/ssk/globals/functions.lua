  ...
  if( type(listener) == "function" ) then
    ...
    listener2 = function( event )
      if( not listener or listener.removeSelf == nil ) then
        ...
        pcall( function() listener[name]( listener, event ) end )
      end
    end
...
function _G.quickLine( group, x, y, len, color, strokeWidth, yOffset )
    color = color or { 1,1,1,1 }
    strokeWidth = strokeWidth or 1
    extraWidth = extraWidth or 0
    yOffset = yOffset or 0
...
_G.shortenString = function( text, maxLen, appendMe )
