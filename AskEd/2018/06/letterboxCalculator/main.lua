...
local function runIt()
	...
	local function onSelect(event)		
		...
		local function onOpenFolder()
			local path = ssk.files.documents.getRoot()			
			ssk.files.desktop.explore(path)
		end
		ssk.misc.easyAlert( "Ideal Recipe Generated",
					           "Your texture is in the documents sandbox folder.",
					           { { "OK", nil }, { "Open Folder", onOpenFolder } } ) 
	end
	--
...
if( fullw == 640 and fullh == 960 ) then 
	runIt()
else
	local tmp = display.newText( "PLEASE VIEW AS: 640 x 960", centerX, centerY - 50, "Poppins-Bold.ttf", 38)
	local tmp = display.newText( "OR TOOL WILL PRODUCE", centerX, centerY, "Poppins-Bold.ttf", 38)
	local tmp = display.newText( "INCORRECT RESULTS", centerX, centerY + 50, "Poppins-Bold.ttf", 38)
end