...
-- ==
--    ssk.math2d.vectorToAngle( ... ) - Converts a screen-space vector to a display object angle (rotation).
-- ==
function math2do.vectorToAngle( ... ) -- ( objA ) or ( x1, y1 )
	local angle
	if( type(arg[1]) == "number" ) then
...
function math2do.vectorToAngleFast( x, y )
	return mCeil(mAtan2( y, x ) * 180 / mPi) + 90
end


-- ==
--    ssk.math2d.angleToVector( angle [ , altRet ]) - Converts a display object angle (rotation) into a screen-space vector.
-- ==
function math2do.angleToVector( angle, tableRet )
...
function math2do.angleToVectorFast( angle )
	local screenAngle = mRad(-(angle+90))
	local x = mCos(screenAngle) 
	local y = mSin(screenAngle) 
	return -x,y
...
-- **** 
-- **** tweenAngle - Delta between an objA and vectorToAngle(objA, objB)
-- ****            - Returns vectorToAngle as second return value (for cases where you need it too) :)
-- **** 
function math2do.tweenAngle( objA, objB )
	local vx,vy      = math2do.sub( objA.x, objA.y, objB.x, objB.y )
	vx,vy            = math2do.normalize(vx,vy)
	local vecAngle   = math2do.vectorToAngle(vx,vy)
...
-- ****   vecAngle - equivalent to objMath2d.vectorToAngle( objA, objB)
...
function math2do.tweenData( objA, objB )
	...
	local vecLen     = math2do.length(vx,vy)
	local vecAngle   = math2do.vectorToAngle(nx,ny)
	local tweenAngle = vecAngle - objA.rotation
