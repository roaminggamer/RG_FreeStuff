...
local gravity = 10 --MOD
local intialForceMultiplier = 1 --MOD
local perFrameDelta = 1.005  --MOD
local forceMultiplier = intialForceMultiplier  --MOD

local physics = require("physics") ; physics.start() ; physics.setGravity( 0,gravity ) ; physics.setDrawMode( "normal" )
...
local function getTrajectoryPoint( startingPosition, startingVelocity, n )
	...
	local stepGravity = { x=t*0, y=t*gravity }  --b2Vec2 stepGravity = t * t * m_world
	return {
...
local function fireProj( event )
	...
	proj:setLinearVelocity( vx * forceMultiplier ,vy * forceMultiplier ) --MOD
...
local function screenTouch( event )
	...
	elseif ( event.phase == "moved" ) then
		...
		updatePrediction( event )
		fireProj( event )
		Runtime:removeEventListener( "enterFrame", enterFrame ) --MOD
	end
	return true
