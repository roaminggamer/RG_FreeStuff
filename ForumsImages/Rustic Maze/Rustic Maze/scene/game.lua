...
function scene:create(event)
	...
	local params = event.params or lastParams; -- RG
	lastParams = params -- RG
...
function scene:resume(buttonName)
	if buttonName == "restart" then
		--RG composer.gotoScene("scene.refresh", {params = {worldNum = world, levelNum = level}});
...
function scene:hide(event)
	...
	for k,v in pairs(event) do
		print( "game scene:hide() ", k, v, system.getTimer() )
	end
