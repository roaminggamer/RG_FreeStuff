...
function joy.create( group, x, y, params )
	if( ssk.__debugLevel > 0 ) then
		print("easyInputs.joystick.create()", x, y )
	end
	...
	if(inputObj) then
		...
		function joystick:touch( event )
			...
			if(percent == 0 ) then
				post(eventName, {angle=angle, vx=vx, vy=vy, nx=nx, ny=ny, percent=percent, phase = event.phase, state = "off", time = getTimer() }, debugLevel)
			else
				post(eventName, {angle=angle, vx=vx, vy=vy, nx=nx, ny=ny, percent=percent, phase = event.phase, state = "on", time = getTimer() }, debugLevel)
			end
	...
	joystick.innerRing = innerRing
	joystick.outerRing = outerRing

	return joystick
end
-- =============================================================
easyInputs.joystick = joy
