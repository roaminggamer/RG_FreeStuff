...
function example.run( group )
	...
	local function onGravityOn( event )
		physics.setGravity(0,5)
		return true
	end
	local gravityOnB = easyIFC:presetPush( group, "default", 
		left + 130, bottom - 100, 250, 50, "Gravity On", onGravityOn, 
		{ labelColor = _G_, labelFont = _G.fontB, labelSize = 36, strokeWidth = 0 } )	


	local function onGravityOff( event )
		physics.setGravity(0,0)
		return true
	end
	local gravityOffB = easyIFC:presetPush( group, "default", 
		left + 130, bottom - 45, 250, 50, "Gravity Off", onGravityOff, 
		{ labelColor = _R_, labelFont = _G.fontB, labelSize = 36, strokeWidth = 0 } )	
