...
function scene:create( event )
   ...
   local title = display.newText( sceneGroup, "Basic Physics Movements", centerX, top + 50, 
                                  native.systemFontBold, 36 )

   local startY = centerY - 7 * 60/2 + 60
   local buttons = {}
   ...
   newButton( "Impulse - Upward Kick", "scripts.impulse" )

   newButton( "Force #1 - Vertical Thrust", "scripts.force" )
   newButton( "Force #2 - Left-Right Thrust", "scripts.force2" )

   newButton( "Angular Impulse", "scripts.angularImpulse" )
   newButton( "Torque", "scripts.torque" )
   
   newButton( "Linear Velocity", "scripts.linearVelocity" )
   newButton( "Angular Velocity", "scripts.angularVelocity" )
