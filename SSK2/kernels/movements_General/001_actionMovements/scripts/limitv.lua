...
function example.run( group )
	...
	target.enterFrame = function( self )
		-- Change direction every 60 counts
		count = count + 1
		if (count%60 == 0 ) then
			dir = -dir 
		end
	...
	player.enterFrame = function( self )
		...
		actions.face( self, { target = target, rate = 180 } )

		-- Thrust forward (rate equates to a mass independent force)
		actions.movep.thrustForward( self, { rate = 100 } )
