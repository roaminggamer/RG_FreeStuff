...
function scene:create( event )
   ...
   local title = display.newText( sceneGroup, "Game Movements", centerX, top + 50, 
                                  native.systemFontBold, 36 )

   local examples = 
   {
      { "RAD #1", "scripts.rad1" },
      { "Asteroids", "scripts.asteroids" },
      { "Lunar Lander #1", "scripts.lunar_lander1" },

   }

   local startY = centerY - #examples * 60/2 + 60
   local buttons = {}
   ...
   for i = 1, #examples do
      newButton( examples[i][1], examples[i][2] )
   end
