...
--  main.lua (35/115)
-- =============================================================
-- Kernel: Escape (by Ketchapp) Movement
-- =============================================================
...
local function createPlayer( x, y )
	...
	player.enterFrame = function( self )
		if( self.thrust ) then
			self.target = -60
			ssk.actions.face( self, { angle = self.target, rate = 90 } )
		else
			--print(self.thrust)
			--self:applyForce( 0, -self.thrust * self.mass, self.x, self.y  )
			self.target = 60
			ssk.actions.face( self, { angle = self.target, rate = 60 } )
		end

		local vx,vy = ssk.math2d.angle2Vector( self.rotation + 90 )
		--print(vx,vy)
		self:setLinearVelocity( 500, vy * 300, self.x, self.y )
...
createHallSegment = function( x, y, preTrigger )
	...
	local trigger = newRect( layers.content, x, centerY,
		{ w = tweenX-4, h = fullh, fill = _Y_, alpha = 0.05, stroke = _Y_, 
