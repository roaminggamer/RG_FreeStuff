...
local html = require "mltext.html"

-- GlitchGames/GGColour
-- https://github.com/GlitchGames/GGColour
local ggcolor = require "mltext.GGColour"
local colorChart = ggcolor:new()
colorChart:loadColours( "mltext/otherRGB.json" )


local function newMLText( parentGroup, words, x, y, params )
	...
	local function onTouch( self, event )
		if(event.phase == "began") then
			for i = 1, self.numChildren do
				if(self[i].isText) then
					self[i]:setFillColor(unpack(params.linkColor1))
				end
			end
		...
		elseif(event.phase == "ended") then
			for i = 1, self.numChildren do
				if(self[i].isText) then
					self[i]:setFillColor(unpack(params.linkColor2))
				end
			end

			if(self.href) then
				--print("OPEN HREF ", self.href )
	...
	r_mltext = function( r_twords, group, r_params  )
		...
		for i = 1, #r_twords do
			...
			if(type(word) == "string") then
				print("EFM", "::"..word.."::", r_params.font, r_params.fontSize, lastX, y)
				tmp = display.newText( group, word, lastX, y, r_params.font, r_params.fontSize)
				table.dump(r_params.fontColor)
				tmp:setFillColor(unpack( r_params.fontColor or _K_) ) 
