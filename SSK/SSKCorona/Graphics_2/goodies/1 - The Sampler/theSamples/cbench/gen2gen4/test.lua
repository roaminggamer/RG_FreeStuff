...
function gameLogic:createScene( screenGroup )
	...
	timer.performWithDelay( 1500, function() theBenchmark( 1 ) end )
end
...
theBenchmark = function( numOps )
	...
	layers.content.x = layers.content.contentWidth
	layers.content.y = layers.content.contentHeight
...
verifyCounts = function()
	...
	if(useOld == true) then
		...
		oldAvgFPS = round(avgFPS,1)
	else
		newDuration = durationMS
		newEventCount = eventCount
		newAvgFPS = round(avgFPS,1)
	end
	...
	if(allowExit) then
		...
		local vs = round(newAvgFPS/oldAvgFPS,2)

		_G.lastResultMessage = rowsCols^2 .. " Sprites rendered @ " .. newAvgFPS .. " FPS vs. " .. oldAvgFPS .. " FPS (" .. vs .. "x)"
