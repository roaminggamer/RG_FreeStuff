...
createButtons = function ( )
	...
	local buttonNames = { "Jerry", "Karla", "Mara", "Penelope", "Ed",
	 "Teresa", "Marcelo", "Margaret", "Laurie", "Jackie", "Heidi"  }

	-- Push Button Test
	local row = 0
	for i = 0, #buttonNames-1 do
		local presetNum = i+1
		if( i % 2 == 0 ) then
			row = row + 1
		end
		...
		tmpButton = ssk.buttons:presetRadio( layers.content, "blueGreenGradient", buttonX, buttonY, buttonWidth, buttonHeight, 
		   buttonNames[presetNum], pbCB )	

		if(buttonNames[presetNum] == "Laurie") then
			tmpButton:toggle()
		end
	end
