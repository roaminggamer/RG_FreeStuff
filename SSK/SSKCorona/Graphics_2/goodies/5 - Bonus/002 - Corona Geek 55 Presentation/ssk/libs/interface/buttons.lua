...
function buttons:new( parentGroup, params )
	...
	buttonInstance.textColor    = fnn(buttonInstance.textColor, {1,1,1,1})
...
if(buttonInstance.selRectEn) then
	...
	labelText:setFillColor( buttonInstance.textColor[1], buttonInstance.textColor[2], 
...
--EFM G2		labelText:setReferencePoint( display.CenterReferencePoint )
	...
	-- ==
	--    buttonInstance:setFillColor( color ) - Set the color of the text on the current button.
	--
	...
	function buttonInstance:setHighlight( vis )	
		...
		if(self.highlighted == nil or self.highlighted ~= vis) then
			...
			if(self.selTextColor) then 
				if(vis) then		
					self.labelText:setFillColor( unpack( self.selTextColor ) )
				else
					self.labelText:setFillColor( unpack( self.textColor ) )
				end
			end
